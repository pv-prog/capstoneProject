...
/**
 * A utility class to encrypt and decrypt credit card numbers using AES encryption.
 * <p>
 * This class provides methods to securely encrypt and decrypt credit card numbers using
 * the AES algorithm in CBC mode with PKCS5 padding. The encryption key is retrieved
 * from the application's configuration properties (via {@link Value} annotation), and the
 * encryption is performed with a randomly generated Initialization Vector (IV) to ensure
 * uniqueness for each encryption.
 * </p>
 * 
 * The class offers two main methods:
 * <ul>
 *     <li>{@link #encrypt(String)}: Encrypts the provided credit card number.</li>
 *     <li>{@link #decrypt(String)}: Decrypts the provided encrypted credit card number.</li>
 * </ul>
 * 
